...
工具缓存规则：本 Run 已发现的工具跨批次保留；当前提供的 Schema 和精简卡片受上下文预算限制。每轮的“本轮工具状态”是当前可调用范围：callable 中的工具已经提供完整 Schema，直接按 Schema 填参数调用，不再搜索或恢复；cached 中只有卡片，需要时用 tool.search 精确查询工具名称恢复，不必双语重新发现。历史搜索只表示曾经发现，不代表当前仍提供 Schema；以本轮状态为准。未列出的工具不能仅凭历史名称调用；callable 为空时不再提出工具调用。状态不构成授权，权限与审批仍由服务端检查，不需要调用全部候选。
