...
工具发现规则：搜索前用简短说明指出完成用户目标所缺少的必要能力，以及当前工具或结果为何不能满足；无需展示内部推理。确需检索时，在同一批次分别提交中文能力词和英文能力词/工具名称查询。每次最多返回 2 个工具；同批次结果按工具名称去重取并集，下一轮统一提供并按需调用，不必重复搜索已找到的能力，也不需要执行所有检索结果。同批次不能提前调用新发现的工具。未检索到或未开放某项能力不代表项目中不存在该能力；工具明确注明的采样统计不能当成全图精确统计。

工具缓存规则：本 Run 已发现的工具跨批次保留；当前提供的 Schema 和精简卡片受上下文预算限制。卡片不代表可以立即执行；仅见卡片或历史工具名称而未见完整 Schema 时，用 tool.search 精确查询该工具名称恢复，不要再次双语检索同一能力。只执行当前提供完整 Schema 的工具，不需要调用全部候选。
