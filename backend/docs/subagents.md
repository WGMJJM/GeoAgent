...
中英文查询放在同一模型工具批次，各自最多返回 2 个工具；同批次所有搜索结果按工具名称去重取并集，通常最多 4 个不同的延迟工具，下一轮统一提供模型。空结果不会覆盖本批次其他有效结果。已发现的工具名称跨批次保留在本 Run 的 Checkpoint 中，不再由新搜索整批替换；没有跨 Run 或跨用户共享发现缓存。
...
只见卡片或历史名称、未见完整 Schema 时，可以用 `tool.search` 精确名称请求复用本 Run 发现记录，经过当前权限/环境校验后优先注入下一轮；这个激活请求仍计入工具调用，不需要重新做双语能力发现。同批次不能抢先执行。原始搜索结果保存于 Checkpoint，模型视图只保留历史命中名称，避免旧卡片描述和本轮 Schema 重复；新 Run 从空发现记录开始。尚未调整原关键词匹配规则：目标不可用时普通检索仍可能返回近似候选，不自动执行替代工具。

主/子 Agent 共用工具选择提示：先对照用户目标、当前工具描述/Schema 和已有观察；已有工具够用就直接调用，结果足够就回答，不能为补充背景扩大分析或列出整个工作区。必要参数缺失时询问用户，权限/参数/临时执行错误不等于缺少能力。确需发现额外能力时先简要说明目标相关的缺口，保留中英文同批次检索及去重，不自动执行全部候选。未搜索到不代表不存在。这些是模型决策约束，不新增语义 Gate、匹配分数阈值或服务端硬性禁止搜索；真实模型仍可能偏离提示。
