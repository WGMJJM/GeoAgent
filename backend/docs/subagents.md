...
`GEOAGENT_TOOL_CONTEXT_TOKENS=3200` 和 `GEOAGENT_TOOL_CONTEXT_MAX_CARDS=8` 集中配置本轮工具上下文预算。最多展示 8 个已发现延迟工具条目（完整 Schema 或卡片，同名不重复），按最近发现/使用顺序优先；常驻工具定义也计入 Schema token 预算，但不属于 8 个延迟工具条目。完整 Schema 与本轮工具状态说明（含卡片）合计计入约 3200 tokens，Schema 放不下时提供精简卡片，卡片也放不下则本轮不展示。预算使用原会话摘要的中英文字符估算，不是对应模型 tokenizer 的精确计数；不计用户消息、记忆或业务工具输出，也不截断单个参数 Schema。常驻定义与状态说明本身超过配置预算时明确报错，不自动扩大预算。
