...
聊天界面在工具调用次数后直接显示 `输入 12.00k · 输出 0.64k`，不再展示总量。1k=1000 tokens，保留两位小数，悬浮说明保留准确原始计数；运行中每轮响应完成后更新，子 Run 用量包含在主 Run。所有已完成调用均返回完整 usage 时优先展示实际量；任一轮缺少完整 usage 则展示全程本地估算并明确标注，不能将部分实际量与估算相加冒充精确量。累计快照按模型调用次数选择，重复或并行晚到事件不重复累加、不倒退，也不替换工具执行进度。刷新后从 Run 持久化字段恢复，旧 Run 无用量不显示虚假的零消耗。

限制：回答正文已接通流式，但正在进行的请求结束前仍无法得到最终 usage；用量在每轮完整响应结束后更新，而非逐 token 增长。失败/超时或进程在响应后、用量入库前中断的调用无法核实实际消耗，不计入本指标。会话摘要等 Run 外模型请求不包含在运行用量中；本指标不是账户账单、费用或整个模型输入总限额。恢复只累加新的已完成响应，待执行工具批次恢复本身不新增模型计数。

## 回答流式输出

AgentLoop 使用现有 `ModelAdapter.stream`，每个正文片段立即经回调和现有 WebSocket `delta` 消息送到聊天界面，不另发非流式模型请求。完整回复只在流的终止片段收到后参与决策、持久化和计量；工具名称与 JSON 参数按调用索引累积到终止片段，随后沿用现有参数校验、权限、Checkpoint 与执行流程。分片参数不执行，工具结束后的总结仍来自下一轮模型请求。

每轮发送 `ModelResponseStarted` 事件，界面清空上一轮临时正文，再逐段追加本轮文字。最终消息持久化后替换临时气泡；“先检查数据”等工具前过程文字不拼入最终回答。子 Agent 沿用同一模型流聚合逻辑，但不把子任务正文混入主会话气泡。恢复已有待执行工具批次时不重新请求模型。

兼容适配器设置 `stream_options={"include_usage": true}`，不会在首次 `finish_reason` 时直接退出，而是继续读取独立统计包或正常 EOF；统计包可能 `choices=[]`，见 [官方流式说明](https://developers.openai.com/cookbook/examples/how_to_stream_completions)。缺失 usage 的兼容服务沿用本地估算展示，真实零值仍有效。原有模型请求超时覆盖整个流；取消、断流、超时或消费端关闭都会关闭响应连接。断流不自动重试、不执行未完成的工具决策、不持久化部分正文为成功回答。

如果配置明确声明模型不支持流式，沿用适配器现有的单片段响应方式；不自动尝试其他协议。正文首片段仍取决于供应商的生成速度，GIS 工具计算期间只有现有进度事件，不能实时显示尚未生成的总结。
...
回答流式接入完整回归：**235 passed，0 failed，15 项已有依赖弃用警告，123.69 秒**。随后补充明确禁用流式及流超时两个场景，适配器文件单独回归 **15 passed**（含上述全量已执行的 13 项），当前共 237 项均已验证；界面 **38 passed**，TypeScript 检查与生产构建通过，修改文件的 Ruff 及 `git diff --check` 通过。复用现有四个测试文件，验证正文早于模型终止到达 WebSocket、分轮清空、最终正文不重复、交错工具参数聚合、终止前不执行工具、尾部/零 usage、取消/断流/超时关闭连接及不支持流式的既有单响应行为。没有额外在线模型请求，未新增永久测试文件。
